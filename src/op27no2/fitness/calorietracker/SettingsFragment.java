    ...
    public void onPause(){
     ...
     /*   int calories = prefs2.getInt("calories", -2000);
		...
		editor.commit();
			...
			if(sizebig == false){
			MyWidgetProvider.pushWidgetUpdate(getActivity().getApplicationContext(), remoteViews, widgetId);
			}
			else if(sizebig == true){
			System.out.println("sizestillbig");
			MyBigWidgetProvider.pushWidgetUpdate(getActivity().getApplicationContext(), remoteViews, widgetId);
			}
        }
